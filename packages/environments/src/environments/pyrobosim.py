...
class PyRoboSimEnv(BaseEnvironment):
    ...
    def _get_move_cost_fn(self):
        def get_move_time(robot, loc_from, loc_to):
            ...
            plan = self.robots[robot].path_planner.plan(from_pose, to_pose)  # ty: ignore[unresolved-attribute]

            # Clear the latest path to avoid showing in the plot
            self.robots[robot].path_planner.latest_path = None  # ty: ignore[invalid-assignment]
...
class MatplotlibWorldCanvas(WorldCanvas):
    ...
    def __init__(self, world, show_plot: bool = True, record_plots: bool = False):
        ...
        self.draw_signal = self.MockSignal(self.draw_signal_callback)  # ty: ignore[invalid-assignment]
        # Add other signals as needed to prevent attribute errors
        self.show_robots_signal = self.MockSignal(self.show_robots)  # ty: ignore[invalid-assignment]
        self.show_planner_and_path_signal = self.MockSignal(self._show_all_paths)  # ty: ignore[invalid-assignment]
