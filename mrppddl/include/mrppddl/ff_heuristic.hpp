...
double ff_heuristic(
  ...
  if (relaxed_result.empty()) return std::numeric_limits<double>::infinity();
  State relaxed = relaxed_result[0].first;

  double dtime = relaxed.time() - t0;
  const auto& initial_fluents = relaxed.fluents();
  std::unordered_set<Fluent> known_fluents(initial_fluents.begin(), initial_fluents.end());
  ...
  while (!newly_added.empty()) {
    ...
    for (const Action* a : all_actions_set) {
      ...
      if (!temp.satisfies_precondition(*a, /*relax=*/true)) continue;
  ...
  while (!needed.empty()) {
    ...
    if (initial_fluents.count(f)) continue;

    auto it = fact_to_action.find(f);
    if (it == fact_to_action.end()) continue;
...
HeuristicFn make_ff_heuristic(
    GoalFn is_goal_fn,
    std::vector<Action> all_actions, FFMemory *memory = nullptr) {
