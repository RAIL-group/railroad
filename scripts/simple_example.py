...
def main():
    ...
    # Create operators
    move_op = environments.operators.construct_move_operator_nonblocking(
        move_time=env.get_move_cost_fn()
    )

    # Search operator with 80% success rate when object is actually present
    def object_find_prob(r, loc, o): return 0.8 if o in OBJECTS_AT_LOCATIONS.get(
        loc, dict()).get("object", dict()) else 0.2
    search_op = environments.operators.construct_search_operator(
        object_find_prob=object_find_prob,
        search_time=lambda r, loc: 5.0
    )

    pick_op = environments.operators.construct_pick_operator_nonblocking(
        pick_time=lambda r, l, o: 5.0
    )

    place_op = environments.operators.construct_place_operator_nonblocking(
